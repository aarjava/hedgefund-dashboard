...
with st.sidebar:
    ...
    vol_q_high = st.slider(
        ...
        DEFAULT_VOL_QUANTILE_HIGH,
        0.05,
        help="Threshold for defining 'High Volatility'. E.g., 0.80 means the top 20% of volatility days are classified as High.",
    )
    ...
    bt_cost = (
        st.number_input(
            ...
            step=1,
            help="Transaction cost in basis points (bps) per trade side. Example: 10 bps = 0.10%.",
        )
        / 10000
    )
    allow_short = st.checkbox(
        "Allow Short Selling?",
        value=False,
        help="If enabled, the strategy will sell short (-100% exposure) when the signal is bearish, instead of going to cash.",
    )
