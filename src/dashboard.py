...
with st.sidebar:
    ...
    if mode == "Single-Asset":
        ...
        adv_pct = st.slider(
            ...
            0.01,
            help="Assumed participation rate in Average Daily Volume (ADV) for liquidity calculations. Limits trade size to X% of daily volume.",
        )
    ...
    vol_q_high = st.slider(
        ...
        DEFAULT_VOL_QUANTILE_HIGH,
        0.05,
        help="Threshold for 'High Volatility' regime. E.g., 0.8 means the top 20% of volatility days are classified as 'High Vol'. Higher value = fewer days in High Vol regime.",
    )
    ...
    if mode == "Single-Asset":
        ...
        bt_cost = (
            st.number_input(
                ...
                step=1,
                help="Simulated transaction cost in basis points (1 bp = 0.01%). E.g., 10 bps = 0.10% per trade.",
            )
            / 10000
        )
        ...
        dd_alert = st.slider(
            ...
            -0.2,
            0.05,
            help="Trigger an alert if the portfolio drawdown exceeds this percentage (negative value).",
        )
        vol_alert = st.slider(
            ...
            0.35,
            0.05,
            help="Trigger an alert if annualized volatility exceeds this value (e.g., 0.3 = 30%).",
        )
        beta_alert = st.slider(
            ...
            1.3,
            0.1,
            help="Trigger an alert if portfolio beta relative to benchmark exceeds this value.",
        )
        dttl_alert = st.slider(
            ...
            5.0,
            1.0,
            help="Trigger an alert if estimated days to liquidate the portfolio (based on ADV participation) exceeds this value.",
        )
