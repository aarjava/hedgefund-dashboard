...
with st.sidebar:
    ...
    if mode == "Single-Asset":
        ...
        adv_pct = st.slider(
            "ADV Participation %", 0.01, 0.30, float(DEFAULT_ADV_PCT), 0.01, help="Max daily volume to trade (e.g., 0.10 = 10%)"
        )
    ...
    vol_q_high = st.slider(
        "High Volatility Quantile", 0.5, 0.95, DEFAULT_VOL_QUANTILE_HIGH, 0.05, help="Threshold for top X% of volatility (e.g., 0.80 = top 20%)"
    )

    if mode == "Single-Asset":
        st.subheader("5. Backtest Settings")
        bt_cost = st.number_input("Transaction Cost (bps)", value=DEFAULT_COST_BPS, step=1, help="Trading cost per turn (e.g., 10 bps = 0.10%)") / 10000
