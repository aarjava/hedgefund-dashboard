...
with st.sidebar:
    ...
    if mode == "Single-Asset":
        ...
        factor_window = st.slider(
            ...
            63,
            7,
            help="Lookback window to calculate rolling betas against factors.",
        )
        vol_window = st.slider(
            ...
            21,
            5,
            help="Lookback period to calculate realized volatility for regime classification.",
        )
        adv_pct = st.slider(
            "ADV Participation %",
            0.01,
            ...
            0.01,
            help="Maximum participation rate in Average Daily Volume (ADV) to estimate liquidity constraints.",
        )
    ...
    vol_q_high = st.slider(
        ...
        DEFAULT_VOL_QUANTILE_HIGH,
        0.05,
        help="Quantile threshold (e.g., 0.95 = top 5%) to define 'High Volatility' regimes.",
    )
    ...
    if mode == "Single-Asset":
        ...
        bt_cost = (
            st.number_input(
                ...
                step=1,
                help="Transaction cost per trade in basis points (1 bp = 0.01%).",
            )
            / 10000
        )
        ...
        dd_alert = st.slider(
            ...
            -0.2,
            0.05,
            help="Trigger an alert if the portfolio drawdown exceeds this threshold.",
        )
        vol_alert = st.slider(
            ...
            0.35,
            0.05,
            help="Trigger an alert if annualized volatility exceeds this threshold.",
        )
        beta_alert = st.slider(
            ...
            1.3,
            0.1,
            help="Trigger an alert if portfolio beta to benchmark exceeds this threshold.",
        )
        dttl_alert = st.slider(
            ...
            5.0,
            1.0,
            help="Trigger an alert if estimated days-to-liquidate the portfolio exceeds this threshold.",
        )
