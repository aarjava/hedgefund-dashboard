...
with st.sidebar:
    ...
    if t_mode == "Preset Universe":
        ...
        ticker = st.text_input("Enter Symbol (Yahoo Finance)", value="NVDA", help="e.g. AAPL, MSFT, BTC-USD").upper()
    ...
    vol_q_high = st.slider("High Volatility Quantile", 0.5, 0.95, 0.75, 0.05, help="Threshold for identifying High Volatility regimes (e.g. 0.75 means top 25% of volatility readings).")
    
    st.subheader("5. Backtest Settings")
    bt_cost = st.number_input("Transaction Cost (bps)", value=10, step=1, help="Basis points per trade (10 bps = 0.1%).") / 10000
    allow_short = st.checkbox("Allow Short Selling?", value=False, help="If enabled, the strategy will sell short when the trend signal is negative.")
