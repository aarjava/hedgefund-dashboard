...
with st.sidebar:
    ...
    vol_q_high = st.slider(
        ...
        DEFAULT_VOL_QUANTILE_HIGH,
        0.05,
        help="Threshold for 'High Volatility'. 0.90 means the top 10% most volatile days.",
    )
    ...
    bt_cost = (
        st.number_input(
            ...
            step=1,
            help="Execution cost per trade. 10 bps = 0.10% drag on returns.",
        )
        / 10000
    )
    allow_short = st.checkbox(
        "Allow Short Selling?",
        value=False,
        help="If checked, strategy will short sell (bet against) the asset when the signal is negative.",
    )
