...
with st.sidebar:
    ...
    vol_q_high = st.slider(
        ...
        DEFAULT_VOL_QUANTILE_HIGH,
        0.05,
        help="Threshold for identifying High Volatility regimes. e.g., 0.80 means the top 20% most volatile days.",
    )
    ...
    bt_cost = (
        st.number_input(
            ...
            step=1,
            help="Trading friction per trade. 10 bps = 0.10%.",
        )
        / 10000
    )
    allow_short = st.checkbox(
        "Allow Short Selling?",
        value=False,
        help="If checked, strategy will short sell when trend is negative.",
    )
