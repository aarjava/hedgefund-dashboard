...
with st.sidebar:
    ...
    if mode == "Single-Asset":
        ...
        factor_window = st.slider(
            ...
            63,
            7,
            help="Lookback window for factor beta estimation (e.g., 63 days = ~3 months).",
        )
        vol_window = st.slider(
            ...
            21,
            5,
            help="Lookback window for historical volatility (e.g., 21 days = ~1 month).",
        )
        adv_pct = st.slider(
            "ADV Participation %",
            0.01,
            ...
            0.01,
            help="Max percentage of average daily volume allowed per trade (e.g., 0.10 = 10%).",
        )
    ...
    vol_q_high = st.slider(
        ...
        DEFAULT_VOL_QUANTILE_HIGH,
        0.05,
        help="Threshold for 'High' volatility regime. E.g., 0.80 means the top 20% most volatile days are classified as High Volatility.",
    )
    ...
    if mode == "Single-Asset":
        ...
        bt_cost = (
            st.number_input(
                ...
                step=1,
                help="Trading friction in basis points. 10 bps = 0.10%.",
            )
            / 10000
        )
        ...
        dd_alert = st.slider(
            ...
            -0.2,
            0.05,
            help="Triggers an alert if drawdown exceeds this threshold (e.g., -0.2 = -20%).",
        )
        vol_alert = st.slider(
            ...
            0.35,
            0.05,
            help="Triggers an alert if annualized volatility exceeds this threshold (e.g., 0.35 = 35%).",
        )
        beta_alert = st.slider(
            ...
            1.3,
            0.1,
            help="Triggers an alert if portfolio beta to benchmark exceeds this threshold.",
        )
        dttl_alert = st.slider(
            ...
            5.0,
            1.0,
            help="Triggers an alert if estimated days to liquidate the portfolio exceeds this value.",
        )
