...
with st.sidebar:
    ...
    vol_q_high = st.slider(
        ...
        help="Percentile threshold (e.g., 0.8 = top 20%) to classify 'High Volatility' regimes."
    )

    st.subheader("5. Backtest Settings")
    bt_cost = st.number_input("Transaction Cost (bps)", value=DEFAULT_COST_BPS, step=1, help="Simulates execution drag. 10 bps = 0.10%. Typical range: 5-20 bps.") / 10000
    allow_short = st.checkbox("Allow Short Selling?", value=False, help="If checked, strategy will short sell when trend is negative. Otherwise, it moves to cash.")
...
with tab_ov:
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df.index, y=df['Close'], name='Close Price', line={'color': 'white', 'width': 1}))
    fig.add_trace(go.Scatter(x=df.index, y=df[f'SMA_{sma_window}'], name=f'{sma_window}-Day SMA', line={'color': '#ff9f43', 'width': 1}))
    ...
    fig.add_trace(go.Scatter(x=high_vol_pts.index, y=high_vol_pts['Close'], mode='markers', name='High Volatility', marker={'color': 'red', 'size': 2}))
...
with tab_bt:
    ...
    if not res_df.empty:
        ...
        # 2. Equity Curve
        fig_eq = go.Figure()
        fig_eq.add_trace(go.Scatter(x=res_df.index, y=res_df['Equity_Strategy'], name='Trend Strategy', line={'color': '#00ff00'}))
        fig_eq.add_trace(go.Scatter(x=res_df.index, y=res_df['Equity_Benchmark'], name='Buy & Hold', line={'color': 'gray', 'dash': 'dot'}))
