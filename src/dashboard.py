...
with st.sidebar:
    ...
    if mode == "Single-Asset":
        ...
        factor_window = st.slider(
            ...
            63,
            7,
            help="Lookback period for computing factor betas (e.g., 63 days = ~3 months).",
        )
        vol_window = st.slider(
            ...
            21,
            5,
            help="Lookback period for computing regime volatility (e.g., 21 days = ~1 month).",
        )
        adv_pct = st.slider(
            "ADV Participation %",
            0.01,
            ...
            0.01,
            help="Average Daily Volume participation limit (e.g., 0.10 = 10% of ADV).",
        )
    ...
    if mode == "Single-Asset":
        ...
        bt_cost = (
            st.number_input(
                ...
                step=1,
                help="Trading cost per transaction in basis points (e.g., 10 bps = 0.10%).",
            )
            / 10000
        )
        ...
        dd_alert = st.slider(
            ...
            -0.2,
            0.05,
            help="Alert threshold for maximum portfolio drawdown (e.g., -0.20 = -20% drawdown).",
        )
        vol_alert = st.slider(
            ...
            0.35,
            0.05,
            help="Alert threshold for annualized portfolio volatility (e.g., 0.35 = 35% volatility).",
        )
        beta_alert = st.slider(
            ...
            1.3,
            0.1,
            help="Alert threshold for portfolio beta relative to benchmark (e.g., 1.3 = 30% more volatile than benchmark).",
        )
        dttl_alert = st.slider(
            ...
            5.0,
            1.0,
            help="Alert threshold for maximum days to liquidate positions based on ADV (e.g., 5.0 = 5 days).",
        )
