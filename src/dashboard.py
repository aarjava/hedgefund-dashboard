...
with st.sidebar:
    ...
    if mode == "Single-Asset":
        ...
        factor_window = st.slider(
            ...
            help="Lookback window for calculating rolling factor betas."
        )
        vol_window = st.slider(
            "Regime Vol Window (days)", 10, 60, 21, 5,
            help="Lookback window for volatility calculation used in regime classification."
        )
        adv_pct = st.slider(
            "ADV Participation %", 0.01, 0.30, float(DEFAULT_ADV_PCT), 0.01,
            help="Percentage of Average Daily Volume (ADV) assumed usable for liquidation without significant price impact."
        )
    ...
    vol_q_high = st.slider(
        ...
        help="Threshold for High Volatility regime (e.g., 0.8 = top 20% of historical volatility days)."
    )
    ...
    if mode == "Single-Asset":
        ...
        bt_cost = st.number_input(
            ...
            step=1,
            help="Trading cost in basis points (1 bp = 0.01%) deducted per turnover."
        ) / 10000
        allow_short = st.checkbox("Allow Short Selling?", value=False)
    else:
        st.subheader("5. Alert Thresholds")
        dd_alert = st.slider("Max Drawdown Alert", -0.6, -0.05, -0.2, 0.05, help="Trigger alert if max drawdown exceeds this level (negative value).")
        vol_alert = st.slider("Volatility Alert (ann.)", 0.1, 1.0, 0.35, 0.05, help="Trigger alert if annualized volatility exceeds this level.")
        beta_alert = st.slider("Beta Alert", 0.5, 2.0, 1.3, 0.1, help="Trigger alert if portfolio beta to benchmark exceeds this level.")
        dttl_alert = st.slider("Days-to-Liquidate Alert", 1.0, 20.0, 5.0, 1.0, help="Trigger alert if days-to-liquidate full portfolio exceeds this threshold.")
