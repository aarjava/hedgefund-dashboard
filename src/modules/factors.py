...
def compute_factor_betas(
    returns: pd.Series, factor_returns: pd.DataFrame, window: int = 63
) -> pd.DataFrame:
    """
...
def compute_factor_contributions(
    beta_df: pd.DataFrame, factor_returns: pd.DataFrame
) -> pd.DataFrame:
    """
...
def compute_alpha_series(
    returns: pd.Series, market_returns: pd.Series, window: int = 63
) -> pd.Series:
    """
